...
namespace CombatCore.Core
{
    ...
    public class HullCollision
    {
            ...
            RigidTransform transform2, NativeHull hull2)
        {
            ...
            // 增加边检测 主要是物体如果两个凸起的地方错开, 实际未相交, 单纯计算面就相交
        ...
        // 查询两个体之间的边距离
        public static unsafe void QueryEdgeDistance(out EdgeQueryResult result, RigidTransform transform1,
            NativeHull hull1, RigidTransform transform2, NativeHull hull2)
        {
            // 在第二个体的局部空间中进行计算
            RigidTransform transform = math.mul(math.inverse(transform2), transform1);
            ...
            for (int i = 0; i < hull1.EdgeCount; i += 2)
            {
                ...
                {
                    NativeHalfEdge* edge2 = hull1.GetEdgePtr(j); // 获取边2
                    NativeHalfEdge* twin2 = hull1.GetEdgePtr(j + 1); // 获取边2的对偶边
        ...
        /// <param name="C1">第一个物体的质心(用于决定分离轴的方向)</param>
        /// <returns>
        ///  返回值为在构造出的分离轴(E1 X E2)方向上, 第二个边相对于第一个边的投影距离:
        ///     若为正, 表示存在间歇
        ///     若为负, 表示存在重叠(即发生碰撞);
        ///     若极小值, 表示两边近似平行, 不适合作为分离轴.
        /// </returns>
        public static float Project(float3 P1, float3 E1, float3 P2, float3 E2, float3 C1)
        {
            ...
            // 步骤2: 判断是否平行(叉积近似为0)
            float kTol = 0.005f;
            float L = math.length(E1_x_E2); // 得到叉积向量的长度, 即sin(θ)·|E1|·|E2|
